model = dict(
    ...
    backbone=dict(
        ...
        # frozen_stages  只要frozen_stages≥0， self.conv1就被冻结, 值是多少，就冻结多少个res_layer层
    cls_head=dict(
        type='I3DHead',
        in_channels=512,
        num_classes=51,
        dropout=0.5),
    test_cfg=dict(average_clips='prob'))    # 表示是对cls_score进行softmax
...
total_epochs = 12
checkpoint_config = dict(interval=1)    # interval表示没多少个epoch，save checkpoints
evaluation = dict(interval=1, metrics=['top_k_accuracy', 'mean_class_accuracy'], topk=(1, 5))
log_config = dict(interval=20, hooks=[dict(type='TextLoggerHook')])
log_level = 'INFO'
work_dir = './work_dirs/posec3d/slowonly_r50_hmdb51_k400p/s1_joint_nopre'
